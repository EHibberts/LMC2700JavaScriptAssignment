...
let highRatings = (rating) => {
    ...
    for(let i = 0; i < allMovies.length; i++){
        if (allMovies[i].rating > rating){
            highRatingsList.push(allMovies[i]);
        }
    }
    for(let i = 0; i < highRatingsList.length; i++){
        console.log(highRatingsList[i].title + " has a rating of " + highRatingsList[i].rating);
    }
    console.log("\nIn total, there are " + highRatingsList.length + " matches")
}
